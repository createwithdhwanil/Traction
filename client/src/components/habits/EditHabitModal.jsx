  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-lg">
        <h2 className="text-2xl font-bold mb-6 dark:text-white">Edit Habit</h2>

        <form onSubmit={submitHandler} className="space-y-4">
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="w-full rounded-xl border p-3 dark:bg-slate-800 dark:text-slate-400 dark:placeholder:text-slate-400"
          />
          ...
          {reminderEnabled && (
            <input
              ...
              className="w-full rounded-xl border p-3 dark:bg-slate-800 dark:text-slate-400 dark:placeholder:text-slate-400 mt-3"
            />
          )}
          <div className="flex justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2 rounded-xl border dark:border-slate-700 dark:text-white"
            >
              Cancel
            </button>
